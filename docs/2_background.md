...
- $\sigma_\text{min}^s$ and $\sigma_\text{max}^s$: its minimum and maximum allowed state-of-charge, expressed as a fraction of the total energy capacity
...
-  $\boldsymbol{p}$: the time series of power sent to the grid 
...
We assume that the time increment $\Delta t$ is sufficiently large to neglect transient effects. Here, the power of each storage system is described by one variable $\boldsymbol{p}^s$. However, the storage power can also be described by its power in charge and discharge separetly, depending on the problem formulation.
...
$$ \forall i \in[0,n], \quad  \sigma_\text{min}^s \bar{E}^s  \leq e_i^s \leq \sigma_\text{max}^s \bar{E}^s$$
...
```{math}
\text{R} = \boldsymbol{\lambda}^T \cdot \boldsymbol{p}
 ```
...
```{note}
The class member {py:obj}`OpSchedule.revenue_storage <components.OpSchedule.revenue_storage>` and {py:obj}`OpSchedule.annual_revenue_storage <components.OpSchedule.annual_revenue_storage>` represent arbitrage revenues, not added revenues.
```
...
```{note}
The class member {py:obj}`OpSchedule.a_npv <components.OpSchedule.a_npv>` models the NPV using arbitrage revenues, and thus does not take into account the contribution of storage to curtailment reduction.

```
It is possible to use a more precise formula of the NPV, with the function {py:obj}`financial_metrics <kernel.financial_metrics>`, taking into account the operational expenditures and the timing of the storage system replacement.
In this case, the total capital expenditures is made of shared costs (represented by $\lambda^\text{shared}_P$ and proportional to the grid connection capacity $\bar{P}$), the capital expenditures of the production and storage components, as
```{math}
\text{CAPEX} = \lambda^\text{shared}_P \bar{P} + \sum_{g} \lambda^g_P \bar{P}^g  +  \sum_{s} (\lambda^s_P \bar{P}^s + \lambda_E^s \bar{E}^s) \sum_{k=0}^{\lfloor m/m_s \rfloor} \dfrac{1}{(1+r)^{k m_\text{s}}}.
```
Each storage system is replaced at the end of its lifetime, as long as it is equal or shorter than the remaining lifetime of the project. In this formula, the replacement costs are discounted.

The operational expenditure of the project are made of fixed ($\lambda_\text{O,fix}^\Box$) and variable ($\lambda_\text{O,var}^\Box$) parts, proportional to the rated power and energy delivered of the component, respectively, 

```{math}
\text{OPEX} = \sum_{g} \lambda_\text{O,fix}^g \bar{P}^g + \lambda_\text{O,var}^g \mathbb{1}^T\cdot\boldsymbol{p}^g + \sum_{s} \lambda_\text{O,fix}^s \bar{P}^s + \lambda_\text{O,var}^s \mathbb{1}^T\cdot \text{max}(0, \boldsymbol{p}^g). 
```
For the storage system, the variable operational expenditures are assumed to be proportional to the energy delivered (i.e. $\boldsymbol{p}^s \geq 0$), not the total energy cycled. Then, the NPV can be calculated as

```{math}
\text{NPV} = - \text{CAPEX} + \sum_{k=1}^{m} \dfrac{ \text{R} - \text{OPEX}}{(1+r)^k}
```
Note that decommissioning costs are not taken into account.

### LCOE

The function {py:obj}`financial_metrics <kernel.financial_metrics>` computes the levelized cost of energy (LCOE) using the following formula,

```{math}
\text{LCOE} = \dfrac{ \sum_{k=1}^{m} \dfrac{\mathbb{1}^T \cdot \boldsymbol{p} \Delta t}{(1+r)^k }}{\text{CAPEX} + \sum_{k=1}^{m} \dfrac{\text{OPEX}}{(1+r)^k}}.
```

### Reliabity

In SHIPP, the term reliability refers to the percentage of time where a specific dispatch constraint  is satified (see below). A dispatch constraint can be expressed as a non-linear equation of the delivered power at each time step,  $g(p_i) \leq 0$. Then, the associated reliability $r$ is expressed as

```{math}
r = \dfrac{1}{n}\sum_{i=0}^{n-1} z_i, \text{   where  } z_i = \begin{cases}
  1 & \text{if } g(p_i) \leq 0, \\
  0 & \text{else}.
\end{cases}
