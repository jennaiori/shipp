...
```python
os = solve_lp_sparse(price_ts, prod, prod_null, stor, stor_null, discount_rate, n_year, p_max, n, p_min, options=dict(fixed_cap = True))
```
...
```python
os = solve_lp_pyomo(price_ts, prod, prod_null, stor, stor_null, discount_rate, n_year, p_max, n, p_min,  options=dict(fixed_cap = True, name_solver = name_solver))
```
...
Similarly, a ramp-limit can be enforced in the dispatch optimization problem, here with the parameters `dp_min` and `dp_max`. 
