...
if pyo_solver == 'none':
    ...
    os =  solve_lp_sparse(price_dam, prod, prod_null, stor, stor_null, discount_rate, n_year, p_max, n, p_min)
    # Solve the dispatch problem with fixed storage capacity
    os_fixed =  solve_lp_sparse(price_dam, prod, prod_null, stor, stor_null, discount_rate, n_year, p_max, n,  p_min, options=dict(fixed_cap = True))

else:
    # Solve the integrated sizing + dispatch problem
    os =  solve_lp_pyomo(price_dam, prod, prod_null, stor, stor_null, discount_rate, n_year, p_max, n, p_min, options = dict(name_solver = pyo_solver))
     # Solve the dispatch problem with fixed storage capacity
    os_fixed =  solve_lp_pyomo(price_dam, prod, prod_null, stor, stor_null, discount_rate, n_year, p_max, n, p_min, options=dict(name_solver = pyo_solver, fixed_cap = True))
