...
res = run_storage_operation('forecast', data_power, data_price, 0, p_max, os.storage_list[0], e_start, n_for, n, dt, rel_th,  forecast_perfect, name_solver = 'mosek', dp_lim = dp_lim, verbose = False, mu = mu, beta_obj = beta_obj, alpha_obj = alpha_obj)
...
for t in range(nt):
    p_vec, e_vec, p_vec2, _, p_cur, bin_vec, status = solve_dispatch_pyomo(data_price[t:], m, rel_th, n_for, forecast_perfect[t], p_min, p_max, e_start_new, 0,  dt, stor, stor_null, n_hist = n_hist, cnt_hist=(t-1), verbose = verbose, name_solver = pyo_solver, dp_lim = dp_lim, beta_obj = beta_obj, mu = mu, p_hist_res = p_hist_res, p_hist_stor=p_hist_stor, alpha_obj = alpha_obj)
